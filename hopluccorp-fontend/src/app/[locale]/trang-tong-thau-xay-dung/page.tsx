        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <FadeIn direction="left" delay={0.3}>
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-6">
                {constructionIntro.title}
              </h2>
              <p className="text-gray-600 leading-relaxed">
                {constructionIntro.description}
              </p>
            </div>
          </FadeIn>
          ...
          <FadeIn direction="right" delay={0.4}>
            <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl">
              <Image
                src={constructionIntro.image}
                alt="Tổng thầu Xây dựng"
                fill
...
function GallerySection() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % constructionGallery.length);
    }, 3000);
  ...
  const getVisibleImages = () => {
    const images = [];
    for (let i = 0; i < 3; i++) {
      const index = (currentIndex + i) % constructionGallery.length;
      images.push({ src: constructionGallery[index], index });
    }
    return images;
  };

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev - 1 + constructionGallery.length) % constructionGallery.length);
  };
